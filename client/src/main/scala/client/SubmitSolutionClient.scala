...
object SubmitSolutionClient extends JSApp {
  ...
  final class Report(reportId: String) extends Observer[Line] {
    ...
    override def onNext(elem: Line): Future[Ack] = {
      def append(text: String, to: Node) =
        to.appendChild(document.createTextNode(s"$text\n"))

      append(elem.value, to = report)
      Continue
    }

    def onComplete() = ()

    def onError(e: Throwable) = System.err.println(s"${this.getClass.getName} $e")
  }
    ...
    def unapply(message: String) = {
      ...
      json.event.asInstanceOf[String] match {
        ...
        case "error" =>
          ...
          throw new SimpleWebSocketClient.Exception(
